...
public class EventLogger implements DemoPlayer.Listener, DemoPlayer.InfoListener,
  ...
  @Override
  public void onStateChanged(boolean playWhenReady, int state, long startTimeFilter) {
    Log.d(TAG + startTimeFilter, "state [" + getSessionTimeString() + ", " + playWhenReady + ", " +
...
//        printStatInfo();
  ...
  private Intent printStatInfo(long startTimeFilter) {
    Log.e("Hongyi_multiplestreaming", "Start Time: " + startTimeFilter);
    ...
    for (Pair<String, Integer> bitrateSample : this.videoBitrateVarience) {
      ...
      bitrateValue[counter] = bitrateSample.second / 1000;  // convert to kbps
      counter++;
    }
  ...
  public void onError(Exception e, long startTimeFilter) {
    Log.e(TAG + startTimeFilter, "playerFailed [" + getSessionTimeString() + "]", e);
  }

  @Override
  public void onVideoSizeChanged(int width, int height, long startTimeFilter) {
    Log.d(TAG + startTimeFilter, "videoSizeChanged [" + getSessionTimeString() + ", " + width + ", " + height + "]");
  }
