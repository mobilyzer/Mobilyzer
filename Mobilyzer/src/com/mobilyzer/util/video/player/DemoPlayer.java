...
public class DemoPlayer implements ExoPlayer.Listener, ChunkSampleSource.EventListener,
   ...
   */
  public interface Listener {
    void onStateChanged(boolean playWhenReady, int playbackState, long startTimeFilter);
    void onError(Exception e, long startTimeFilter);
    void onVideoSizeChanged(int width, int height, long startTimeFilter);
  }
...
//    void onBandwidthSample(int elapsedMs, long bytes, long bitrateEstimate);
  ...
  public DemoPlayer(RendererBuilder rendererBuilder, long startTimeFilter) {
    this.rendererBuilder = rendererBuilder;
  ...
  /* package */ void onRenderersError(Exception e) {
    ...
    for (Listener listener : listeners) {
      listener.onError(e, this.startTimeFilter);
    }
    rendererBuildingState = RENDERER_BUILDING_STATE_IDLE;
    maybeReportPlayerState();
  }
  ...
  public void onPlayerError(ExoPlaybackException exception) {
    rendererBuildingState = RENDERER_BUILDING_STATE_IDLE;
    for (Listener listener : listeners) {
      listener.onError(exception, this.startTimeFilter);
    }
  }
  ...
  public void onVideoSizeChanged(int width, int height) {
    for (Listener listener : listeners) {
      listener.onVideoSizeChanged(width, height, this.startTimeFilter);
    }
  }
...
//  public void onBandwidthSample(int elapsedMs, long bytes, long bitrateEstimate) {
  ...
  private void maybeReportPlayerState() {
    ...
    if (lastReportedPlayWhenReady != playWhenReady || lastReportedPlaybackState != playbackState) {
      for (Listener listener : listeners) {
        listener.onStateChanged(playWhenReady, playbackState, this.startTimeFilter);
      }
      lastReportedPlayWhenReady = playWhenReady;
      lastReportedPlaybackState = playbackState;
    }
